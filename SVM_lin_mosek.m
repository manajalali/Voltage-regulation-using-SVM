function [a,b,et]=SVM_lin_mosek(C,N,mu_lin,z_kernel,qg_max,T,yt,epsilon)
...
% 0<= qg_max_n-(K_n a_n +b_n *1)=z && 0<= qg_max_n+(K_n a_n +b_n *1)=x
...
prob.c   = [zeros(1,(T*N)) zeros(1,N) (1/T)*ones(1,T) mu_lin*ones(1,N) zeros(1,N*T) zeros(1,T) zeros(1,N*T)];
% prob.c   = [zeros(1,(T*N)) (1/T)*ones(1,T) mu_lin*ones(1,N) zeros(1,N*T) zeros(1,T) zeros(1,N*T)];
...
% prob.blx = [-inf*ones(1,(T*N)) zeros(1,T) -inf*ones(1,T+2*N*T+N)]';
...
[r,res]=mosekopt('minimize param',prob)
...
gama_N=res.sol.itr.xx(N*T+N+T+1:N*T+N+T+N);
a=reshape(a,T,N);
a=value(a);
b=value(b);
et=value(et);
